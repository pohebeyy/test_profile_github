...
st.title("Оценка GitHub-проекта")
st.write("Модель оценивает, насколько проект выглядит живым, понятным и готовым для показа.")
...
if submit:
    ...
    if result["strong_project"]:
        st.success("Проект выглядит сильно: его можно смело показывать.")
    else:
        st.warning("Проект пока слабый: добавь README, демо, тесты или свежие коммиты.")
